...
export class GridComponent implements OnInit {
  ...
  col: any[] = [];
  box: any[] = [];
  errorCells: any[] = []; // This Doesn't error cells but highlights cells the are already there when user enter same value in different row. 
  ...
  highlightBox(I: number, J: number, cor: number) { //This function highlight all the duplicae numbers. 

    if (this.row[I][cor] == 1) {
      let otherJ = this.sudokugrid[I].findIndex((x: string) => parseInt(x) == cor + 1);
      this.errorCells[I][otherJ] = 1;
      setTimeout(() => {
        this.errorCells[I][otherJ] = 0;
      }, 500)

    }
    if (this.col[J][cor] == 1) {
      let otherI = this.sudokugrid.findIndex((x: string[]) => parseInt(x[J]) == cor + 1);
      this.errorCells[otherI][J] = 1;
      setTimeout(() => {
        this.errorCells[otherI][J] = 0;
      }, 500)
    };

    if (this.box[this.getBox([I, J])][cor] == 1) {
      let boxNumber = this.getBox([I, J])
      for (let i = 0; i < 9; i++) {
        for (let j = 0; j < 9; j++) {
          if (this.getBox([i, j]) == boxNumber) {
            if (parseInt(this.sudokugrid[i][j]) == cor + 1) {        
              this.errorCells[i][j] = 1;
              setTimeout(() => {
                this.errorCells[i][j] = 0;
              }, 500)
              return;
            }
          }
